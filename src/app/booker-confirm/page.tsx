import type { Metadata } from "next"
import BookerConfirm from "./client"

export const metadata: Metadata = {
  title: "Become a Booker",
  description: "Become a booker on Nigeria's premier event management platform.",
  keywords: [
    "Spotix booker",
    "Become a booker Spotix",
    "event organizer Nigeria",
    "ticket booking login Nigeria",
    "Spotix account access",
  ],
  openGraph: {
    title: "Become a Booker",
    description: "Sign back into your Spotix Account. Access your events, tickets, and bookings.",
    url: "https://spotix.com.ng/booker-confirm",
    siteName: "Spotix Nigeria",
    type: "website",
    images: [
      {
        url: "https://i.postimg.cc/FR5xpcpZ/hero.jpg",
        width: 1200,
        height: 630,
        alt: "Booker Creation - Become a Booker",
      },
    ],
  },
  twitter: {
    card: "summary_large_image",
    title: "Become a Booker",
    description: "Sign back into your Spotix Account. Access your events, tickets, and bookings.",
    images: ["https://i.postimg.cc/FR5xpcpZ/hero.jpg"],
  },
  alternates: {
    canonical: "https://spotix.com.ng/booker-confirm",
  },
  robots: {
    index: true,
    follow: true,
  },
}

export default function BookerConfirmPage() {
  return <BookerConfirm />
}